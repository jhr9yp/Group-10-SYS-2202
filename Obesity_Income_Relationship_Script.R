...
df_2015<- aggregate(df_2015$Data_Value, by = list(df_2015$Income), mean)
df_2016<- aggregate(df_2016$Data_Value, by = list(df_2016$Income), mean)
df_avg<- aggregate(df_avg$Data_Value, by = list(df_avg$Income), mean)
...
ggplot(data = df_2015) +
  ...
  labs(x = "Income", y = "Obesity Level", title = "2015 Obesity and Income Relationship")+
  theme(
...
ggplot(data = df_2016) +
  geom_point(mapping = aes(x = Group.1, y = x)) +
  labs(x = "Income", y = "Obesity Level", title = "2016 Obesity and Income Relationship")+
  theme(
    axis.title.x = element_text(size = 14, face = "bold.italic"),
    axis.title.y = element_text(size = 14, face = "bold.italic"),
    title = element_text(size = 20, face = "bold")
...
ggplot(data = df_avg) +
  geom_point(mapping = aes(x = Group.1, y = x)) +
  labs(x = "Income", y = "Obesity Level", title = "2015-2016 Obesity and Income Relationship")+
  theme(
    axis.title.x = element_text(size = 14, face = "bold.italic"),
    axis.title.y = element_text(size = 14, face = "bold.italic"),
    title = element_text(size = 20, face = "bold")
